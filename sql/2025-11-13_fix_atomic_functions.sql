-- Fix both work_order_create_atomic and work_order_update_atomic
-- Issues fixed:
-- 1. CREATE: Missing userId field in INSERT statement
-- 2. UPDATE: Missing type field in cash_transactions INSERT statements

-- ============================================================================
-- FIX 1: work_order_create_atomic - Add userId to INSERT
-- ============================================================================
...
-- ============================================================================
-- FIX 2: work_order_update_atomic - Add type field to cash_transactions INSERT
-- ============================================================================
...
BEGIN
  ...
  -- STEP 1: Restore stock for removed/reduced parts
  ...
  LOOP
    ...
    -- Find if this part still exists in new list
    SELECT jsonb_agg(elem) INTO v_new_part
    FROM jsonb_array_elements(COALESCE(p_parts_used, '[]'::jsonb)) elem
    WHERE elem->>'partId' = v_part_id;

    IF v_new_part IS NULL THEN
      -- Part removed completely, restore full quantity
      UPDATE parts
      SET stock = jsonb_set(stock, ARRAY[v_branch_id], to_jsonb(COALESCE((stock->>v_branch_id)::int, 0) + v_old_quantity), true)
      WHERE id = v_part_id;
      ...
      VALUES (
        ...
        public.mc_avg_cost(v_part_id, v_branch_id) * v_old_quantity,
        v_branch_id,
        'Hoàn trả từ cập nhật phiếu',
        p_order_id
      );
    ELSE
      -- Part still exists, check quantity diff
      v_quantity := COALESCE((v_new_part->0->>'quantity')::int, 0);
      v_quantity_diff := v_old_quantity - v_quantity;

      IF v_quantity_diff > 0 THEN
        -- Reduced quantity, restore difference
        UPDATE parts
        SET stock = jsonb_set(stock, ARRAY[v_branch_id], to_jsonb(COALESCE((stock->>v_branch_id)::int, 0) + v_quantity_diff), true)
        WHERE id = v_part_id;
  ...
  -- STEP 2: Deduct stock for new/increased parts
  FOR v_index IN 0..(v_parts_count - 1) LOOP
    ...
    SELECT COALESCE((elem->>'quantity')::int, 0) INTO v_old_quantity
    FROM jsonb_array_elements(COALESCE(v_old_parts, '[]'::jsonb)) elem
    WHERE elem->>'partId' = v_part_id
    LIMIT 1;

    v_quantity_diff := v_quantity - COALESCE(v_old_quantity, 0);

    IF v_quantity_diff > 0 THEN
      -- New part or increased quantity
